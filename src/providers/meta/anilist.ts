...
class Anilist extends AnimeParser {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          animeInfo.mappings = anifyInfo.mappings.filter(
            (item: any) =>
              !(item as { img: string }).img.startsWith('https://media.kitsu.app'),
          );
  ...
  private findAnimeSlug = async (
    ...
    if (
      malId &&
      !(this.provider instanceof Crunchyroll || this.provider instanceof Bilibili)
    ) {
      ...
      const results: IAnimeInfo[] = data?.map((item: any) => {
        return {
          ...
          malId: item.mappings.find(
            (item: any) => item.providerType === 'META' && item.providerId === 'mal',
          )?.id,
          title: {
            romaji: item.title?.romaji,
            english: item.title?.english,
          ...
          episodeTitle:
            item.episodes.latest.latestTitle ?? `Episode ${item.currentEpisode}`,
   ...
   */
  // Assuming you have the necessary imports for axios and cheerio
  GogoAnimeEpisodes = async (slug) => {
    try {
      let Episode = [];
  ...
  fetchEpisodesListById = async (id, dub = false, fetchFiller = false) => {
    const options = {
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
    ...
    if (
      ...
      try {
        // Use GogoAnimeEpisodes to fetch the episodes
        const tempid = Media.title.english.replaceAll('-', '').replaceAll(' ', '-');
        possibleAnimeEpisodes = await GogoAnimeEpisodes(tempid);
    ...
    if (fetchFiller) {
      ...
      if (!fillerData.toString().startsWith('404')) {
        fillerEpisodes = fillerData.episodes || [];
      }
    }
    ...
    possibleAnimeEpisodes = possibleAnimeEpisodes.map((episode) => {
      ...
      if (
        ...
        if (fillerEpisodes[episode.number - 1])
          episode.isFiller = fillerEpisodes[episode.number - 1]['filler-bool'];
      }
